...
function live() {
    ...
    document.getElementById('S').addEventListener('ended', () => {
        ...
        while(fa){
            ...
            try {
                ...
                console.log('try1')
                var f = (parseInt(snkey) + 1).toString()
                if ((parseInt(snkey) + 1).toString().length < 6) {
                    for (var i = (parseInt(snkey) + 1).toString().length; i < 6; i++) {
                        f = '0' + f
                    }
                }
                firebase.database().ref(document.getElementById('devices-list').value + '/Audio/' + dt + '/Online/' + f + '/').once('value', (sn) => {
    ...
    var i = 1;
    var dataq = ''
    var k = 'data:audio/wav;base64,'
    firebase.database().ref(document.getElementById('devices-list').value + '/Audio/' + dt + '/Online/').once('child_added', (sn) => {
        audio.src = "data:audio/wav;base64," + sn.val().toString().replace("Data:", "")
        audio.play()
        snkey = sn.key
    })
